...
for (i in 1:10){
  ...
  mygbm <- randomForest(x = train_feature, y = label_train, ntree =100)
...
train_file_name = paste0(nowtimestring, glue::glue("_train_{splitratio}_random_forest.csv"))
train_file_path = paste0(dir.temp,"/",train_file_name )


test_file_name = paste0(nowtimestring, glue::glue("_test_{splitratio}_random_forest.csv"))
...
result["mean_sd"] <- add_entry(result["mean_sd"],glue::glue("random_forest"), glue::glue("train_{splitratio}"), glue::glue("{mean_train_str} ({sd_train_str})"))
...
result["mean_sd"] <- add_entry(result["mean_sd"],glue::glue("random_forest"), glue::glue("test_{splitratio}"), glue::glue("{mean_test_str} ({sd_test_str})"))
...
result["median_iqr"] <- add_entry(result["median_iqr"],glue::glue("random_forest"), glue::glue("train_{splitratio}"), glue::glue("{median_train_str} ({iqr_train_str})"))
...
result["median_iqr"] <- add_entry(result["median_iqr"],glue::glue("random_forest"), glue::glue("test_{splitratio}"), glue::glue("{median_test_str} ({iqr_test_str})"))
