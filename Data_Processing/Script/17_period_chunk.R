...
timeseries_period_chunks <- array(0, dim = c(subject_size, 86))
...
calculate_period_chunk <- function(arr, chunk) {
    periods <- c()
    i <- 1
    while (i * chunk <= length(arr)) {
        start <- (i - 1) * chunk + 1
        end <- i * chunk
        subarr <- arr[start:end]
        periods <- c(periods, mean(calculate_period(subarr)))
        i <- i + 1
    }
    return(periods)
}
...
for (i in 1:subject_size) { # length of subject_list
    ...
    if (file.exists(fname_timeseries)) {
        ...
        count <- count + 1
        timeseries_period_chunks[count, ] <- apply(
            resr1_lr, 2,
            FUN = function(v) {
                return(max(calculate_period_chunk(v, 100)))
            }
        )
    }
    print(fname_timeseries)
}
save(timeseries_period_chunks, file = paste0(
    dir_processing,
    "/Output/period_chunk.RData"
))
